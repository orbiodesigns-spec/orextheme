...
const ThemeEditorPanel: React.FC<Props> = ({ children, onReset, onSave, isSaving = false, isOpen = true, setIsOpen }) => {

    // Fallback toggle for when setIsOpen is not provided, though it normally should be
    const handleToggle = () => {
        if (setIsOpen) {
            setIsOpen(!isOpen);
        }
    ...
    return (
        <>
            ...
            <button
                ...
                className={`fixed top-28 right-4 z-40 p-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-white transition-all duration-300 ${isOpen ? 'translate-x-[340px] opacity-0 pointer-events-none' : 'translate-x-0'}`}
            >
            ...
            {/* Main Panel */}
            <div
                className={`fixed top-28 right-4 z-40 w-80 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 flex flex-col transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) ${isOpen ? 'translate-x-0 opacity-100' : 'translate-x-[120%] opacity-0'
                    }`}
