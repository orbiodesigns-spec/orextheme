...
interface NavbarProps {
    currentPage?: 'home' | 'pricing' | 'support' | 'store' | 'installation' | 'changelogs' | 'downloads' | 'dashboard';
    onLoginClick: () => void;
    user?: User | null;
}
            ...
            <div className="max-w-7xl mx-auto flex items-center justify-between relative">
                ...
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:flex items-center gap-8">
                    ...
                    <button
                        ...
                        className={`text-sm font-medium transition-colors ${currentPage === 'changelogs' ? 'text-white' : 'text-gray-300 hover:text-white'
                            }`}
                    >
                        Changelogs
                    </button>
                    {user && (
                        <button
                            ...
                            className={`text-sm font-medium transition-colors ${currentPage === 'downloads' ? 'text-white' : 'text-gray-300 hover:text-white'
                                }`}
                        >
                            Downloads
                        </button>
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden absolute top-full left-0 right-0 bg-black/95 backdrop-blur-xl border-b border-white/10 p-6 space-y-4 animate-in slide-in-from-top-4">
                    <div className="flex flex-col gap-4">
                        ...
                        <button
                            onClick={() => { navigate('/changelogs'); setIsMobileMenuOpen(false); }}
                            className={`text-lg font-medium transition-colors text-left ${currentPage === 'changelogs' ? 'text-blue-400' : 'text-gray-300'}`}
                        >
                            Changelogs
                        </button>
                        {user && (
                            <button
                                onClick={() => { navigate('/downloads'); setIsMobileMenuOpen(false); }}
                                className={`text-lg font-medium transition-colors text-left ${currentPage === 'downloads' ? 'text-blue-400' : 'text-gray-300'}`}
                            >
                                Downloads
                            </button>
                        )}
