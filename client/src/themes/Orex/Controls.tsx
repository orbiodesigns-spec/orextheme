...
const Controls: React.FC<ControlsProps> = ({ theme, setTheme }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Background Settings */}
            <div className="space-y-3 pt-2 border-t border-gray-100 pb-4">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Background Settings</h3>
                <div className="space-y-3">
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700 flex justify-between">
                            <span>Image Scale</span>
                            <span className="text-gray-500">{theme.previewBgScale ?? 1}x</span>
                        </label>
                        <input
                            type="range"
                            min="0.1"
                            max="3"
                            step="0.1"
                            value={theme.previewBgScale ?? 1}
                            onChange={(e) => handleChange('previewBgScale', e.target.value)}
                            className="w-full"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700 flex justify-between">
                            ...
                            <span className="text-gray-500">{theme.previewBgPositionX ?? 50}%</span>
                        </label>
                        <input
                            type="range"
                            min="0"
                            max="100"
                            step="1"
                            value={theme.previewBgPositionX ?? 50}
                            onChange={(e) => handleChange('previewBgPositionX', e.target.value)}
                            className="w-full"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700 flex justify-between">
                            ...
                            <span className="text-gray-500">{theme.previewBgPositionY ?? 50}%</span>
                        </label>
                        <input
                            type="range"
                            min="0"
                            max="100"
                            step="1"
                            value={theme.previewBgPositionY ?? 50}
                            onChange={(e) => handleChange('previewBgPositionY', e.target.value)}
                            className="w-full"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700 flex justify-between">
                            ...
                            <span className="text-gray-500">{theme.previewBgBlur ?? 0}px</span>
                        </label>
                        <input
                            type="range"
                            min="0"
                            max="20"
                            step="1"
                            value={theme.previewBgBlur ?? 0}
                            onChange={(e) => handleChange('previewBgBlur', e.target.value)}
                            className="w-full"
                        />
                    </div>
                </div>
            ...
            <div className="flex justify-end pt-2">
                <button
                    onClick={() => {
                        setTheme(prev => ({
                            ...prev,
                            // Swap Team A (Blue) with Team B (Purple)
                            bluePrimary: prev.purplePrimary,
                            blueSecondary: prev.purpleSecondary,
                            purplePrimary: prev.bluePrimary,
                            purpleSecondary: prev.blueSecondary,
                        }));
                    }}
                    className="flex items-center gap-2 px-4 py-2 text-xs font-bold uppercase tracking-wider bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-all shadow-lg shadow-blue-900/20 border border-blue-500/50"
                >
            ...
            {/* Mandala Settings */}
            <div className="space-y-3 pt-2 border-t border-gray-100">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Mandala Settings</h3>
                <div className="space-y-3">
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700">Opacity (0 to 1)</label>
                        <input
                            type="range"
                            min="0"
                            max="1"
                            step="0.1"
                            value={theme.mandalaOpacity ?? 0.2}
                            onChange={(e) => handleChange('mandalaOpacity', e.target.value)}
                            className="w-full"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-gray-700">Rotation Speed (Seconds)</label>
                        <input
                            type="number"
                            value={theme.mandalaSpeed ?? 10}
                            onChange={(e) => handleChange('mandalaSpeed', e.target.value)}
                            className="w-full p-2 border border-gray-300 rounded text-sm"
                        />
                        <p className="text-xs text-gray-500">Lower is faster, Higher is slower</p>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3 pt-2 border-t border-gray-100">
                ...
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        ...
                        <input
                            type="color"
                            value={theme.backgroundColor ?? '#090f31'}
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            type="color"
                            value={theme.glassPanelLeftColor || '#ffffff'}
                            onChange={(e) => handleChange('glassPanelLeftColor', e.target.value)}
                            className="w-8 h-8 rounded cursor-pointer border-none"
                        />
                    </div>
                    <div className="flex flex-col gap-1">
                        ...
                        <input
                            type="color"
                            value={theme.glassPanelRightColor || '#ffffff'}
                            onChange={(e) => handleChange('glassPanelRightColor', e.target.value)}
                            className="w-8 h-8 rounded cursor-pointer border-none"
                        />
                    </div>
                    <div className="flex items-center justify-between">
