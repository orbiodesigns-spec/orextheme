...
const StorePage: React.FC<Props> = ({ user, onLoginClick }) => {
    ...
    const handleApplyCoupon = async () => {
        ...
        setCouponError('');
        try {
            const res = await api.verifyCoupon(couponCode, { productId: selectedProduct.id.toString() });
            setDiscount(res);
        } catch (err: any) {
    ...
    const handleProceedToPay = async () => {
        ...
        try {
            ...
            // 2. Create Order
            const orderData = await api.createProductOrder(selectedProduct.id.toString(), user.phone_number || '', discount?.code);
                ...
                handler: async function (response: any) {
                    try {
                        ...
                        if (verifyRes.status === 'SUCCESS') {
                            ...
                            setSelectedProduct(null); // Close checkout modal
                            setDiscount(null);
                            setCouponCode('');
                        }
                    } catch (err) {
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white font-sans pt-20">
            ...
            {selectedProduct && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-zinc-900 border border-emerald-500/50 rounded-2xl max-w-md w-full p-6 relative">
                        <button
                        ...
                        <p className="text-emerald-400 mb-6">{selectedProduct.name}</p>
                        ...
                        <div className="space-y-4 mb-6">
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="bg-black border border-white/10 rounded-lg px-3 py-2 text-white flex-1 focus:border-emerald-500 outline-none uppercase"
                                    value={couponCode}
                            ...
                            {discount && (
                                <div className="flex justify-between items-center text-green-400">
                                    ...
                                    <span>-₹{(selectedProduct.price - getFinalPrice()).toFixed(2)}</span>
                                </div>
                            )}
                        ...
                        >
                            {purchasing === selectedProduct.id ? 'Processing...' : `Pay ₹${getFinalPrice().toFixed(2)}`}
                        </button>
                    </div>
                </div>
            )}
