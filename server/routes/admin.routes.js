...
router.put('/settings', verifyToken, async (req, res) => {
    ...
    try {
        const promises = Object.keys(settings).map(key => {
            return db.query("INSERT INTO settings (key_name, value) VALUES (?, ?) ON DUPLICATE KEY UPDATE value = ?", [key, settings[key], settings[key]]);
        });
...
// 27. ADMIN: MANAGE INSTALLATION VIDEOS
router.get('/installation-videos', verifyToken, async (req, res) => {
    try {
...
router.post('/installation-videos', verifyToken, async (req, res) => {
    const { title, video_url, thumbnail_url, display_order } = req.body;
    try {
        await db.query(
            "INSERT INTO installation_videos (title, video_url, thumbnail_url, display_order) VALUES (?, ?, ?, ?)",
            [title, video_url, thumbnail_url, display_order || 0]
        );
...
router.put('/installation-videos/:id', verifyToken, async (req, res) => {
    const { id } = req.params;
    const { title, video_url, thumbnail_url, display_order } = req.body;
    ...
    try {
        await db.query(`UPDATE installation_videos SET ${fields.join(', ')} WHERE id = ?`, values);
...
router.delete('/installation-videos/:id', verifyToken, async (req, res) => {
    const { id } = req.params;
    try {
        await db.query("DELETE FROM installation_videos WHERE id = ?", [id]);
...
// 28. ADMIN: MANAGE CHANGELOGS
router.get('/changelogs', verifyToken, async (req, res) => {
    try {
        const [rows] = await db.query("SELECT * FROM changelogs ORDER BY release_date DESC, created_at DESC");
        res.json(rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
router.post('/changelogs', verifyToken, async (req, res) => {
    const { version, description, pdf_url, release_date } = req.body;
    try {
        await db.query(
            "INSERT INTO changelogs (version, description, pdf_url, release_date) VALUES (?, ?, ?, ?)",
            [version, description, pdf_url, release_date]
        );
        res.json({ success: true, message: 'Changelog added' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

router.delete('/changelogs/:id', verifyToken, async (req, res) => {
    const { id } = req.params;
    try {
        await db.query("DELETE FROM changelogs WHERE id = ?", [id]);
